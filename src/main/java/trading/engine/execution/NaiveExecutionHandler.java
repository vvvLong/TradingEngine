...
public class NaiveExecutionHandler implements ExecutionHandler {
    ...
    @Override
    public FillEvent execute(OrderEvent event) {
        String symbol = event.getSymbol();
        LocalDate nextDay = event.getTimestamp().plus(((HistoricalMySQLDataHandler) data).getTimeDelta());
        // get the next open
        ...
        return new FillEvent(
                nextDay,
                symbol,
                event.getDirectionType(),
                filledPrice,
                event.getQuantity(),
                Math.max(10, filledPrice * event.getQuantity() * 0.01),
                "NYSE");
    }
}
